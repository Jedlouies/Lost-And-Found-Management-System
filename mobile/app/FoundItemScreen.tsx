  ...
  return (
    <TouchableOpacity style={styles.itemCard} onPress={() => onCardPress(item)}>
      ...
      <View style={styles.cardDetails}>
        ...
        <Text style={styles.itemDescription} numberOfLines={2}>{item.howItemFound}</Text>
...
export default function FoundItemsScreen() {
  ...
  const onCardPress = (item) => {
    router.push({ pathname: `/items/${item.id}`, params: { type: 'found' } });
  };

  return (
    <SafeAreaView style={styles.mainContainer}>
      <FoundHeader userData={userData} />
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No found items found.</Text>}
        />
      )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <FlatList
              ...
              keyExtractor={item => item.id}
              renderItem={({ item }) => {
                const dateKey = item.dateFound ? 'dateFound' : 'dateLost'; 
