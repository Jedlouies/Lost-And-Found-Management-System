...
function ItemViewDetailsPage() {
  ...
  const renderProfile = (profile, label, linkUid, isGuest = false) => {
    if (isGuest) {
      return (
    ...
    if (!profile?.firstName && !profile?.lastName && !profile?.profileURL) {
      ...
      <div className="item-view-profiles-container">
        ...
        <div
          style={{
            ...
            alignItems: "center",
            gap: "10px",
            cursor: linkUid ? "pointer" : "default",
          }}
          onClick={() => linkUid && navigate(`/admin/view-profile/${linkUid}`)}
        >
          {profile?.profileURL ? (
            <img
    ...
    <>
      ...
      <div className="manage-item-body">
        ...
        <div className="manage-item-container">
          ...
          <div
            className="more-details-information"
            style={{
              position: "absolute",
              top: "10%",
              left: "50%",
          ...
          >
            {type === "lost" && (
              <>
                ...
                {renderProfile(item.personalInfo, "Owner", item.uid, item.isGuest)}
                {renderProfile(item.foundBy, "Founder", item.foundBy?.uid)}
              </>
            )}
            ...
            {type === "found" && (
              <>
                ...
                {renderProfile(item.personalInfo, "Founder", item.uid, item.isGuest)}
                {renderProfile(item.claimedBy, "Owner", item.claimedBy?.uid)}
              </>
            )}
          </div>
        </div>
