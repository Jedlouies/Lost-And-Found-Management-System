...
import '../user_components/styles/HomeHeader.css'
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      await signOut(auth);
      [
        "address","bio","contactNumber","coverURL","designation","educationalAttainment",
        "email","firstName","gender","lastName","middleName","profileURL","role","studentId",
        "uid","yearsOfService","hideAddInfoPanel"
...
<div className='dropdown-containers' style={{ position: 'relative', left: '90%', zIndex: 1000 }}>
  ...
  {dropDown && (
    ...
    >
      ...
      <div className={`account-dropdown-row ${location.pathname === `/admin/settings/${user?.uid}` ? 'active' : ''}`} onClick={() => handleNavigate(`/admin/settings/${user?.uid}`)}>
