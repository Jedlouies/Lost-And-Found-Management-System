...
import { db, resend } from "./firebaseAdmin.js"; // adjust path as needed

cron.schedule("*/15 * * * *", async () => {
  console.log("⏳ Running background job...");

  const now = Date.now();
  const snapshot = await db.collection("itemManagement")
  ...
  snapshot.forEach(async (docSnap) => {
    ...
    if (timeLeft <= 60 * 60 * 1000 && !data.notified60) {
      await realtimeDb.ref(`notifications/${data.uid}`).push({
        message: "⏰ 1 hour left before your item expires!",
        read: false,
        createdAt: new Date().toISOString(),
      });
      ...
      if (data.email) {
        await resend.emails.send({
          from: "Spotsync <Onboarding@spotsync.site>",
          to: data.email,
          subject: "Reminder: 1 hour left before your item expires",
          html: `<p>Hello,</p>
                 <p>Your item <b>${data.itemName || "No name"}</b> will expire in 1 hour.</p>
                 <p>Please take action soon!</p>`,
        });
      }

      await docSnap.ref.update({ notified60: true });
    }

    if (timeLeft <= 30 * 60 * 1000 && !data.notified30) {
      await realtimeDb.ref(`notifications/${data.uid}`).push({
        message: "⏳ 30 minutes left before your item expires!",
        read: false,
        createdAt: new Date().toISOString(),
      });
      ...
      if (data.email) {
        await resend.emails.send({
          from: "Spotsync <Onboarding@spotsync.site>",
          to: data.email,
          subject: "Reminder: 30 minutes left before your item expires",
          html: `<p>Hello,</p>
                 <p>Your item <b>${data.itemName || "No name"}</b> will expire in 30 minutes.</p>
                 <p>Please take action soon!</p>`,
        });
      }

      await docSnap.ref.update({ notified30: true });
    }

    if (timeLeft <= 0 && data.status === "Pending") {
      await docSnap.ref.update({ status: "Canceled" });

      await realtimeDb.ref(`notifications/${data.uid}`).push({
        message: "❌ Your item has expired and is now canceled.",
        read: false,
        createdAt: new Date().toISOString(),
      });
      ...
      if (data.email) {
        await resend.emails.send({
          ...
          html: `<p>Hello,</p>
                 <p>Your item <b>${data.itemName || "No name"}</b> has expired and was canceled.</p>`,
        });
      }
    }
  });
